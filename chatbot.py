...
TYPE_SIZES = {
    ...
    "photo": (768, 768), "pattern": (1024, 1024), "banner": (1200, 630),
    "avatar": (512, 512), "poster": (768, 1024), "general": (768, 768),
}

IMAGE_KEYWORDS = [
    "génère","générer","crée","créer","dessine","dessiner","fais","faire",
    "montre","image","photo","logo","icône","illustration","bannière",
    "affiche","poster","avatar","motif","pattern","visuel","dessin","portrait",
    "generate","create","draw","make","show","picture","design","render","imagine",
]

IMAGE_TYPE_KEYWORDS = {
    "logo":         ["logo","marque","brand"],
    "icon":         ["icône","icon","pictogramme"],
    "illustration": ["illustration","illustre","dessine","art"],
    ...
    try:
        resp = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"""Is this an image generation request? Message: {user_message}

If yes: {{"is_image_request":true,"type":"logo|icon|illustration|photo|pattern|banner|avatar|poster|general","visual_prompt":"english prompt","confirmation_message":"short french message"}}
    ...
    if TOGETHER_API_KEY:
        try:
            r = requests.post(
                ...
                json={"model": "black-forest-labs/FLUX.1-schnell-Free", "prompt": full_prompt,
                      "width": min(width,1024), "height": min(height,1024), "steps": 4, "n": 1, "response_format": "b64_json"},
                timeout=90
            )
            r.raise_for_status()
...
def chat():
        ...
        try:
            ...
            with open(path, "rb") as f:
                transcription = client.audio.transcriptions.create(
                    model="whisper-large-v3", file=("audio.wav", f, "audio/wav"),
                    language="fr", response_format="text"
                )
            os.unlink(path)
            ...
            if not user_message or not user_message.strip():
                return jsonify({"response": "❌ Transcription vide, réessaie."})
        except Exception as e:
            print("[AUDIO ERROR]", e)
    ...
    if user_message.strip():
        ...
        if intent:
            ...
            if img:
                return jsonify({"response": intent["confirmation_message"], "has_image": True,
                                "image_base64": img, "image_type": intent["type"], "visual_prompt": intent["visual_prompt"]})
    ...
    try:
        r = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "Tu es Yelen AI, assistant IA africain intelligent, chaleureux et concis."},
                *chat_history[-6:]
            ],
            temperature=0.7,
            max_tokens=600,
